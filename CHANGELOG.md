# CHANGELOG

- `2026-03-26` **fix: resolve Issue #2 — add missing boss_agent.py and requirements.txt** (Jack Chang)
- `2026-03-25` **[Bot] 自動更新索引與 CHANGELOG** (Naomi Bot)
- `2026-03-26` **feat: S26 BossAgent + full frontend overhaul** (Jack Chang)
- `2026-03-25` **[Bot] 更新系統進化狀態** (Naomi Bot)
- `2026-03-26` **feat: add 13 new squad system prompts (S13-S25)** (Jack Chang)
- `2026-03-25` **[Bot] 自動更新索引與 CHANGELOG** (Naomi Bot)
- `2026-03-25` **[Bot] 更新首頁執行進度儀表板** (Naomi Bot)
- `2026-03-26` **feat: add 11 squad system prompts + fire safety + urban renewal laws** (Jack Chang)
- `2026-03-25` **[Bot] 更新系統進化狀態** (Naomi Bot)
- `2026-03-25` **[Bot] 更新首頁執行進度儀表板** (Naomi Bot)
- `2026-03-26` **feat: add Taiwan building regulations database - all 22 counties/cities + BTR + EEWH** (Jack Chang)
- `2026-03-25` **[Bot] 更新首頁執行進度儀表板** (Naomi Bot)
- `2026-03-25` **[Bot] 更新系統進化狀態** (Naomi Bot)
- `2026-03-26` **ui: remove backend endpoint input from sidebar, settings-only config** (Jack Chang)
- `2026-03-25` **[Bot] 更新首頁執行進度儀表板** (Naomi Bot)
- `2026-03-26` **feat: add CORS middleware for GitHub Pages frontend** (Jack Chang)
- `2026-03-25` **[Bot] 自動更新索引與 CHANGELOG** (Naomi Bot)
- `2026-03-26` **feat: add web frontend demo for GitHub Pages** (Jack Chang)
- `2026-03-23` **[Bot] 自動更新索引與 CHANGELOG** (Naomi Bot)
- `2026-03-24` **feat: 新增 CODEOWNERS，頂層架構限 Fuyo0426 審核，Squad 資料夾開放社群** (Jack Chang)
- `2026-03-23` **[Bot] 自動更新索引與 CHANGELOG** (Naomi Bot)
- `2026-03-23` **[Bot] 更新首頁執行進度儀表板** (Naomi Bot)
- `2026-03-23` **feat: 進度儀表板改為逐Squad追蹤，標示開放貢獻欄位** (Jack Chang)
- `2026-03-23` **[Bot] 自動更新索引與 CHANGELOG** (Naomi Bot)
- `2026-03-23` **[Bot] 更新首頁執行進度儀表板** (Naomi Bot)
- `2026-03-23` **[S00] 建立 11 個 Squad 獨立資料夾（含四層結構前綴）** (Jack Chang)
- `2026-03-23` **[Bot] 自動更新索引與 CHANGELOG** (Naomi Bot)
- `2026-03-23` **觸發 workflow 測試** (Jack Chang)
- `2026-03-23` **[Bot] 自動更新索引與 CHANGELOG** (Naomi Bot)
- `2026-03-23` **修正 workflow：git stash 保存 unstaged changes 再 pull** (Jack Chang)
- `2026-03-23` **修正 workflow：git pull 移至 git add 之前，避免 staged changes 衝突** (Jack Chang)
- `2026-03-23` **修正 workflow YAML：if 條件移至 job 層（頂層不支援）** (Jack Chang)
- `2026-03-23` **[Bot fix] 修正三個 workflow：跳過Bot commit + concurrency排隊 + push前先pull** (Jack Chang)
- `2026-03-23` **[Bot] 更新首頁執行進度儀表板** (Naomi Bot)
- `2026-03-23` **[Bot] 更新首頁執行進度儀表板** (Naomi Bot)
- `2026-03-23` **[Bot] 更新首頁執行進度儀表板** (Naomi Bot)
- `2026-03-23` **[Bot] 更新首頁執行進度儀表板** (Naomi Bot)
- `2026-03-23` **[Bot] 自動更新索引與 CHANGELOG** (Naomi Bot)
- `2026-03-23` **[Bot] 自動更新索引與 CHANGELOG** (Naomi Bot)
- `2026-03-23` **[Bot] 自動更新索引與 CHANGELOG** (Naomi Bot)
- `2026-03-23` **[Bot] 自動更新索引與 CHANGELOG** (Naomi Bot)
- `2026-03-23` **[Bot] 自動更新索引與 CHANGELOG** (Naomi Bot)
- `2026-03-23` **[Bot] 自動更新索引與 CHANGELOG** (Naomi Bot)
- `2026-03-23` **[Bot] 自動更新索引與 CHANGELOG** (Naomi Bot)
- `2026-03-23` **[Bot] 更新首頁執行進度儀表板** (Naomi Bot)
- `2026-03-23` **[Bot] 自動更新索引與 CHANGELOG** (Naomi Bot)
- `2026-03-23` **[Bot] 自動更新索引與 CHANGELOG** (Naomi Bot)
- `2026-03-23` **[Bot] 自動更新索引與 CHANGELOG** (Naomi Bot)
- `2026-03-23` **[Bot] 自動更新索引與 CHANGELOG** (Naomi Bot)
- `2026-03-23` **[Bot] 自動更新索引與 CHANGELOG** (Naomi Bot)
